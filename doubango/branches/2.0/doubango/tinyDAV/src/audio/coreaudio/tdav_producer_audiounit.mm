...
#define kWebRtcVadMode      2 // 0 1 2 3

// 2015.07.30 MICHAEL ADD BEGIN
static VadInst *vadHandle = nil;

// Returns true if the rate and frame length combination is valid.
static bool ValidRatesAndFrameLengths(int rate, int frame_length) {
    if (rate == 8000) {
...
static int webRtcVadProcess(uint32_t inSampleRate, int16_t* audioFrame, int inNumberFrames)
{
    if (!vadHandle) {
        return -1;
    }
    int result = WebRtcVad_Process(vadHandle,
                             (int)inSampleRate,
                             audioFrame,
                             (int)inNumberFrames);
    /*
    if (ValidRatesAndFrameLengths((int)inSampleRate, (int)inNumberFrames)) {
        result = WebRtcVad_Process(vadHandle,
                                   (int)inSampleRate,
                                   audioFrame,
                                   (int)inNumberFrames);
        if (result == 1) {
...
static OSStatus __handle_input_buffer(void *inRefCon,
	...
	if(status == 0){
        ...
        if (avail <= 0) {
            TSK_DEBUG_ERROR("**** speex_buffer_get_available return : %d", (int)status);
        }
        while (producer->started && avail >= producer->ring.chunck.size) {
            int readSize = speex_buffer_read(producer->ring.buffer, (void*)producer->ring.chunck.buffer, (int)producer->ring.chunck.size);
            avail -= readSize;
            ...
            // 2015.07.30 MICHAEL ADD BEGIN
            static bool isVoiceDetected = false;
            static int  noVoiceCount = 0;
            static int  hasVoiceCount = 0;
            // int count = (int)producer->ring.chunck.size / 320;
            if (producer->ring.chunck.size >=  480) {
                int vadResult = webRtcVadProcess(TMEDIA_PRODUCER(producer)->audio.rate, (int16_t*)producer->ring.chunck.buffer, 480);
                if (vadResult == 1) { // has voice
#if (TARGET_OS_IPHONE | TARGET_IPHONE_SIMULATOR)
                    hasVoiceCount++;
                    noVoiceCount = 0;
                    if (!isVoiceDetected && hasVoiceCount > 10) {
                        isVoiceDetected = true;
                        NSDictionary *dic = [NSDictionary dictionaryWithObject:[NSNumber numberWithInteger:1] forKey:@"voiceDetected"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:@"YBLocalVoiceDetection" object:nil userInfo:dic];
                    }
#endif
                }
                else {
                    ...
                    noVoiceCount++;
                    hasVoiceCount = 0;
                    if (isVoiceDetected && noVoiceCount > 80) {
                        isVoiceDetected = false;
                        NSDictionary *dic = [NSDictionary dictionaryWithObject:[NSNumber numberWithInteger:0] forKey:@"voiceDetected"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:@"YBLocalVoiceDetection" object:nil userInfo:dic];
                    }
#endif
                }
        ...
        TSK_DEBUG_ERROR("**** AudioUnitRender return : %d", (int)status);
    }
...
static int tdav_producer_audiounit_start(tmedia_producer_t* self)
{
    ...
    if (!vadHandle) {
        ...
        if (vadHandle) {
            result = WebRtcVad_Init(vadHandle);
            if (result == 0) {
                WebRtcVad_set_mode(vadHandle, kWebRtcVadMode); // Moderate Likelihood
            }
        }
    }
    // 2015.07.30 MICHAEL ADD END
...
{ 
	...
	if(producer){
		...
		if(producer->started){
			tdav_producer_audiounit_stop((tmedia_producer_t*)self);
		}
