...
static tsk_size_t sgLastRetSize = 0;
static UInt64 sgNoAudioCount = 0;
...
static OSStatus __handle_output_buffer(void *inRefCon, 
	...
	for(int i=0; i<ioData->mNumberBuffers; i++){
        // MOD MICHAEL 2015.07.24 BEGIN
		tsk_size_t retSize = tdav_consumer_audiounit_get(consumer, ioData->mBuffers[i].mData, ioData->mBuffers[i].mDataByteSize);
        if (retSize <= 0) { // no audio
            sgNoAudioCount++;
            if (sgLastRetSize > 0 && sgNoAudioCount > 20)
            {
                sgLastRetSize = 0;
                // post notification to main thread (no audio data)
#if (TARGET_OS_IPHONE | TARGET_IPHONE_SIMULATOR)
                NSDictionary *dic = [NSDictionary dictionaryWithObject:[NSNumber numberWithInteger:0] forKey:@"audioLength"];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"YBLocalAudioOutDataChanged" object:nil userInfo:dic];
#endif
            ...
            if (sgLastRetSize <= 0)
            {
                sgLastRetSize = retSize;
                sgNoAudioCount = 0;
                // post notification to main thread (have audio data)
#if (TARGET_OS_IPHONE | TARGET_IPHONE_SIMULATOR)
                NSDictionary *dic = [NSDictionary dictionaryWithObject:[NSNumber numberWithInteger:sgLastRetSize] forKey:@"audioLength"];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"YBLocalAudioOutDataChanged" object:nil userInfo:dic];
#endif
            }
        }
        // MOD MICHAEL 2015.07.24 END
	}
...
{ 
	...
	if(consumer){
		...
		if(consumer->started){
			tdav_consumer_audiounit_stop((tmedia_consumer_t*)self);
		}
		// destroy handle
		if(consumer->audioUnitHandle){
			tdav_audiounit_handle_destroy(&consumer->audioUnitHandle);
