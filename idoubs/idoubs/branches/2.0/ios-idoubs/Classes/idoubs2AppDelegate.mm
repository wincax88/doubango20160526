...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions { 
	...
	//...etc etc etc
    /*
    NSString *path = getCachePath4Audio();
    NSDictionary *dic = [NSDictionary dictionaryWithObject:@"0" forKey:@"audioLength"];
    [dic writeToFile:path atomically:NO];

    [[NSNotificationCenter defaultCenter] postNotificationName:@"AudioOutChanged" object:nil userInfo:dic];
     */
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onLocalAudioOutDataChanged:) name:@"YBLocalAudioOutDataChanged" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onLocalVoiceDetection:) name:@"YBLocalVoiceDetection" object:nil];
    
    return YES;
}

NSString* getCachePath4Audio()
{
    static NSString *cachePath4Audio;
    if (cachePath4Audio.length > 0) {
        return cachePath4Audio;
    }
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    cachePath4Audio = [paths objectAtIndex:0];
    BOOL isDir = NO;
    NSError *error;
    if (! [[NSFileManager defaultManager] fileExistsAtPath:cachePath4Audio isDirectory:&isDir] && isDir == NO) {
        [[NSFileManager defaultManager] createDirectoryAtPath:cachePath4Audio withIntermediateDirectories:NO attributes:nil error:&error];
    }
    cachePath4Audio = [cachePath4Audio stringByAppendingPathComponent:@"audioOut.dat"];
    return cachePath4Audio;
}
...
- (void)onLocalAudioOutDataChanged:(NSNotification*)notification
{
    NSDictionary *userData = notification.userInfo;
    NSNumber *audioLength = [userData objectForKey:@"audioLength"];
    if (audioLength.integerValue <= 0) {
        NSLog(@"+++++++ no audio");
    }
    else {
        NSLog(@"audio found");
...
- (void)onLocalVoiceDetection:(NSNotification*)notification
{
    NSDictionary *userData = notification.userInfo;
    NSNumber *voiceDetected = [userData objectForKey:@"voiceDetected"];
    if (voiceDetected.integerValue <= 0) {
        NSLog(@"onLocalVoiceDetection --> silent");
    }
    else {
        NSLog(@"onLocalVoiceDetection --> talking");
    }
}
